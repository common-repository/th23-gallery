...
class th23_gallery_admin extends th23_gallery_pro {
	...
	function default_size_field($args) {
		...
		foreach($this->data['default_sizes'] as $size_slug => $size_title) {
			echo '<option value="' . esc_attr($size_slug) . '" ' . selected($this->options['default_size'], $size_slug, false) . '>' . esc_html($size_title) . '</option>';
		}
		echo '</select>';
		echo '<p class="description">' . esc_html__('Default image size for newly created galleries', 'th23-gallery') . '</p>';
	}
