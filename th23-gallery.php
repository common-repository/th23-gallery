...
Description: Replace standard Gallery block and legacy [gallery] shortcode handler. Open galleries using PhotoSwipe (https://photoswipe.com). Add [th23-gallery-random-image] shortcode to show a random image out of a given selection.
Version: 4.0.0
...
class th23_gallery {
	...
	function __construct() {
		...
		// Load plugin options
		$this->options = get_option('th23_gallery');
	...
	*/
		...
		*/
		...
		if(!isset($this->data['js']) || empty($this->data['js'])) {
			...
			wp_localize_script('th23-gallery-js', 'th23_gallery_js', array(
				...
				*/
				/* translators: parses in start/end tags to ling "The image" */
	...
	function th23_gallery_block($block_content, $block, $instance) {
		...
		$size = (!empty($block['attrs']['sizeSlug'])) ? $block['attrs']['sizeSlug'] : $this->options['default_size'];
